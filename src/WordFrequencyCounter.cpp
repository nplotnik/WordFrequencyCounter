...
public:
	...
	void calculateWords()
	{
		...
		for (int i = 0; i < d_inputData.size(); ++i)
		{
			...
			{
				std::string currentWord(d_inputData.begin() + startIndex, d_inputData.begin() + startIndex + length);
...
private:
	...
	{
		bool operator()(const Frequency& firstFrequency, const Frequency& secondFrequency)
		{
	...
	std::priority_queue<Frequency, std::vector<Frequency>, CustomComparator> d_sortedFrequencies;
};

std::ostream& operator<<(std::ostream& out, const std::vector<TextProcessor::Frequency> frequencies)
{
